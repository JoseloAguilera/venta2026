...
<div class="dashboard-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="content-area">
            <div class="card mb-4">
                <div class="card-body">
                    <form method="GET" action="<?= base_url('reports/profit-by-item') ?>" class="row g-3">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="itemProfitTable" class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>Artículo</th>
                                    <th>Código</th>
                            ...
                            <tbody>
                                ...
                                foreach ($items as $item): 
                                    $profit = $item['total_amount'] - $item['total_cost'];
                                    $margin = $item['total_amount'] > 0 ? ($profit / $item['total_amount']) * 100 : 0;
                                    
                                    $grandQty += $item['total_qty'];
                                    $grandTotal += $item['total_amount'];
                                    $grandCost += $item['total_cost'];
                                    $grandProfit += $profit;
                                ?>
                                    <tr>
                                        ...
                                        <td><?= formato_moneda($item['total_amount']) ?></td>
                                        <td><?= formato_moneda($item['total_cost']) ?></td>
                            ...
                            <tfoot class="table-dark">
                                <tr>
                                    ...
                                    <td><strong><?= number_format($grandQty, 0) ?></strong></td>
...
<script>
    $(document).ready(function () {
        $('#itemProfitTable').DataTable({
            'order': [[3, 'desc']], // Order by Total Amount descending
            'language': {
